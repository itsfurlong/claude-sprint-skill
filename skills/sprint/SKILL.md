...
- Where the code actually lives (local clone path, or the repo connection in use) and the rule for editing it — targeted string replacement, never full-file regeneration through an API that has no partial-patch mode. Full-file rewrites of anything nontrivial have a real history of silently corrupting an unrelated line; targeted edits don't have that failure mode.
